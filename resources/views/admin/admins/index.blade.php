            ...
            <div class="content-body">
                <section id="file-export">
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                 @if(admin()->check_route_permission('admin.admins.index') == 1)
                                <div class="card-header" style="padding-bottom: 0px">
                                    <h4 class="card-title">{{__('msg.admins')}}</h4>
                                    ...
                                    <div class="heading-elements">
                                        <ul class="list-inline mb-0">
                                            ...
                                            <li>
                                                <a href="{{route('admin.admins.create')}}" style="border-radius: 10px;padding: 10px"
                                                ...
                                                @foreach($admins as $admin)
                                                    <tr class="row_{{$admin->id}}">
                                                        ...
                                                        <td class="text-center">
                                                            @if(admin()->check_route_permission('admin.admins.update.status') == 1)
                                                                <div class="form-group pb-1">
                                                                    <input type="checkbox" admin_id="{{$admin->id}}" name="active"
                                                                           class="switch active_admin" id="switch{{$admin->id}}"
                                                                        {{$admin->active == 1 ? 'checked' : ''}}/>
                                                                </div>
                                                            @else
                                                                {{$admin->active == 1 ? __('msg.active') : __('msg.inactive')}}
                                                            @endif
                                                        </td>
                                                        <td class="d-flex align-items-center justify-content-sm-center">
                                                            @if(admin()->check_route_permission('admin.admins.delete') == 1)
                                                            <a href="#" admin_id="{{$admin->id}}" class="admin_delete btn-sm btn btn-danger">
