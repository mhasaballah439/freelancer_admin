            ...
            <div class="content-body">
                <section id="file-export">
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                @if(admin()->check_route_permission('admin.categories.index') == 1)
                                    <div class="card-header" style="padding-bottom: 0px">
                                        <h4 class="card-title">{{__('msg.category')}}</h4>
                                        <a class="heading-elements-toggle"><i
                                                class="la la-ellipsis-v font-medium-3"></i></a>
                                        <div class="heading-elements">
                                            <ul class="list-inline mb-0">
                                                @if(admin()->check_route_permission('admin.categories.create') == 1)
                                                    <li>
                                            ...
                                            <table class="table table-striped table-bordered file-export">
                                                ...
                                                @foreach($categories as $category)
                                                    <tr class="row_{{$category->id}}">
                                                        ...
                                                        <td class="text-center">
                                                            @if(admin()->check_route_permission('admin.categories.update.status') == 1)
                                                                <div class="form-group pb-1">
                                                                    <input type="checkbox"
                                                                           category_id="{{$category->id}}" name="active"
                                                                           class="switch active_category"
                                                                           id="switch{{$category->id}}"
                                                                        {{$category->active == 1 ? 'checked' : ''}}/>
                                                                </div>
                                                            @else
                                                                {{$category->active == 1 ? __('msg.active') : __('msg.inactive')}}
                                                            @endif
                                                        </td>

                                                        <td class="d-flex align-items-center justify-content-sm-center">
                                                            @if(admin()->check_route_permission('admin.categories.delete') == 1)
                                                                <a href="#" category_id="{{$category->id}}"
                                                                   class="delete_category btn btn-danger btn-sm">
                                                                    <i class="la la-trash"></i></a>
                                                            @endif

                                                            @if(admin()->check_route_permission('admin.categories.edit') == 1)
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('change', '.active_category', function () {
            var category_id = $(this).attr('category_id');
