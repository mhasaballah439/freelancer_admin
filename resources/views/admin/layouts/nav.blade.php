<nav class="header-navbar navbar-expand-md navbar navbar-with-menu navbar-without-dd-arrow fixed-top navbar-semi-light bg-info navbar-shadow">
    <div class="navbar-wrapper">
        ...
        <div class="navbar-container content" style="background-color: #32ebd0 !important;">
                  ...
                  <img src="{{admin()->avatar}}" alt="avatar"><i></i></span>
                    ...
                    <li class="dropdown dropdown-notification nav-item">
                        <a class="nav-link nav-link-label" href="#" data-toggle="dropdown"><i class="ficon ft-bell"></i>
                            <span class="badge badge-pill badge-default badge-danger badge-default badge-up badge-glow count_notif">{{count($notifacation)}}</span>
                        </a>
                            ...
                            <li class="scrollable-container media-list w-100">
                                ...
                                @if(count($notifacation) > 0)
                                   @foreach($notifacation as $item)
                                        <a href="{{$item->type_id == 1 ? route('admin.projects.edit',$item->project_id) : ($item->type_id == 2 ? route('admin.contacts.show',$item->contact_id) : '#')}}">
                                            <div class="media">
                                                <div class="media-left align-self-center"><i class="ft-plus-square icon-bg-circle bg-cyan"></i></div>
                                                <div class="media-body">
                                                    <p class="notification-text font-small-3 text-muted">{{$item->notify}}</p>
                                                    <small>
                                                        <time class="media-meta text-muted">{{$item->created_at->diffForHumans()}}</time>
                                                    </small>
                                                </div>
                                            </div>
                                        </a>
